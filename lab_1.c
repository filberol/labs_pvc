...
/**
  ...
  * If no LICENSE file comes with this software, it is provided AS-IS.qwa
  *
  ******************************************************************************
  */
...
/* USER CODE BEGIN PV */
uint8_t correctCode[CODE_LENGTH] = {1, 0, 1, 0, 1, 0, 1, 0};  // Пример правильного кода
...
void unlock() {
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_SET);  // Зелёный светодиод
  ...
  if(HAL_GetTick() - lastPressTime > 50) {  // debounce
    ...
    if (buttonState && !lastState) {
      ...
      if (pressType == correctCode[currentIndex]) {
        blinkLed(GPIOD, GPIO_PIN_14, 1, 100);  // Жёлтый светодиод на верный ввод
        ...
        blinkLed(GPIOD, GPIO_PIN_15, 1, 200);  // Красный светодиод на ошибку
        errorCount++;
        resetCode();
	if (errorCount >= MAX_ERRORS) {
			  blinkLed(GPIOD, GPIO_PIN_15, 5, 500);  // Красный мигает 5 раз на блокировку
			  errorCount = 0;
			  resetCode();
			}
		  }
